...
def get_loader(cfg):
    ...
    test_dataset_expand = SampleDataset(full_dataset=instantiate_from_config(cfg.test_dataset.Mix), interval=20)
    test_dataset = torch.utils.data.ConcatDataset([test_dataset, test_dataset_expand])
    test_dataset_expand = SampleDataset(full_dataset=instantiate_from_config(cfg.test_dataset.Mix), interval=35)
    test_dataset = torch.utils.data.ConcatDataset([test_dataset, test_dataset_expand])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--results_folder', type=str, default='./', help='None for saving in wandb folder.')
