...
from denoisingdiffusionpytorch.denoising_diffusion_pytorch.simple_diffusion import *
...
class CondGaussianDiffusion(GaussianDiffusion):
    ...
    def p_losses(self, x_start,y_start, times, cond_img, trace,seg=None, extra_cond1=None,extra_cond2=None, noise=None, *args, **kwargs):
        ...
        if self.loss_type == 'l2':
            return F.mse_loss(gt_pred, target)
        elif self.loss_type == 'l1':
            return F.l1_loss(gt_pred, target)
        elif self.loss_type == 'l1+l2':
            return F.mse_loss(gt_pred, target) + F.l1_loss(gt_pred, target)
    ...
    @torch.no_grad()
    def sample(self, cond_img, trace,extra_cond1=None,extra_cond2=None, verbose=True):
        b, c, h, w = cond_img.shape
        extra_cond1 = default(extra_cond1, torch.zeros((b, self.extra_channels, h, w), device=self.device))
        extra_cond2 = default(extra_cond2, torch.zeros((b, self.extra_channels, h, w), device=self.device))
        return self.p_sample_loop((b, self.channels, self.image_size, self.image_size),
                                  cond_img=cond_img,trace=trace,
                                  extra_cond1=extra_cond1, extra_cond2=extra_cond2,verbose=verbose)
    ...
    def p_sample_loop(self, shape, cond_img,trace, extra_cond1,extra_cond2, verbose=True):
        ...
        for i in tqdm(range(self.num_sample_steps), desc='sampling loop time step', total=self.num_sample_steps,
            ...
            img,de = self.p_sample(img,de, conditioning_features,conditioning_features_trace, extra_cond1,extra_cond2, times, times_next)

        img.clamp_(-1., 1.)
        img = unnormalize_to_zero_to_one(img)
        de.clamp_(-1., 1.)
        de = unnormalize_to_zero_to_one(de)
        return img,de

    @torch.no_grad()
    def p_sample(self, x,y, cond,trace, extra_cond1,extra_cond2, time, time_next):
        batch, *_, device = *x.shape, x.device

        model_mean1,model_mean2, model_variance = self.p_mean_variance(x=x,y=y, cond=cond, trace=trace,extra_cond1=extra_cond1,extra_cond2=extra_cond2, time=time,
                                                          time_next=time_next)

        if time_next == 0:
            return model_mean1,model_mean2

        noise = torch.randn_like(x)
        return model_mean1 + sqrt(model_variance) * noise,model_mean2 + sqrt(model_variance) * noise

    def p_sample_g(self, x, y,cond, trace,extra_cond, time, time_next):
        batch, *_, device = *x.shape, x.device

        model_mean, model_variance = self.p_mean_variance(x=x, y=y, cond=cond,trace=trace, extra_cond=extra_cond, time=time,
                                                          time_next=time_next)
    ...
    def p_mean_variance(self, x,y, cond, trace,extra_cond1,extra_cond2, time, time_next):
        ...
        model_mean1 = alpha_next * (x * (1 - c) / alpha + c * x_start)
        model_mean2 = alpha_next * (y * (1 - c) / alpha + c * y_start)

        posterior_variance = squared_sigma_next * c

        return model_mean1,model_mean2, posterior_variance
