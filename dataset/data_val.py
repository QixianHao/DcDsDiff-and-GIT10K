...
def cv_random_flip(img, label,de):
    flip_flag = random.randint(0, 3)
    if flip_flag == 1:
        img = img.transpose(Image.FLIP_LEFT_RIGHT)
        label = label.transpose(Image.FLIP_LEFT_RIGHT)
        de = de.transpose(Image.FLIP_LEFT_RIGHT)
    elif flip_flag == 2:
        img = img.transpose(Image.FLIP_TOP_BOTTOM)
        label = label.transpose(Image.FLIP_TOP_BOTTOM)
        de = de.transpose(Image.FLIP_TOP_BOTTOM)
    elif flip_flag == 3:
        img = img.transpose(Image.FLIP_LEFT_RIGHT)
        label = label.transpose(Image.FLIP_LEFT_RIGHT)
        de = de.transpose(Image.FLIP_LEFT_RIGHT)
...
def randomCrop(image, label,de):
    ...
    return image.crop(random_region), label.crop(random_region),de.crop(random_region)



def randomRotation(image, label,de):
    mode = Image.BICUBIC
    if random.random() > 0.8:
        random_angle = np.random.randint(-15, 15)
        image = image.rotate(random_angle, mode)
        label = label.rotate(random_angle, mode)
        de = de.rotate(random_angle, mode)
    return image, label,de
...
class PolypObjDataset(data.Dataset):
    def __init__(self, image_root, gt_root, de_root,trace_root,trainsize, mean=None, std=None, randomPeper=True,
        ...
        self.gts = [gt_root + f for f in os.listdir(gt_root) if f.endswith('.jpg')
                    or f.endswith('.png')]
        self.des = [de_root + f for f in os.listdir(de_root) if f.endswith('.jpg')
                    or f.endswith('.png')]
        self.traces = [trace_root + f for f in os.listdir(trace_root) if f.endswith('.jpg')
                       or f.endswith('.png')]
        self.images = sorted(self.images)
        self.gts = sorted(self.gts)
        self.des = sorted(self.des)
        self.traces = sorted(self.traces)
        ...
        self.gt_transform = transforms.Compose([
            ...
            transforms.ToTensor()])
        self.de_transform = transforms.Compose([
            transforms.Resize((self.trainsize, self.trainsize)),
            transforms.ToTensor()])
    ...
    def __getitem__(self, index):
        ...
        # data augumentation
        # image, gt,de = self.aug_transform(image=np.asarray(image), mask=np.asarray(gt), de_mask=np.asarray(de)).values()
        # image, gt,de = albumentations.PadIfNeeded(*image_size[::-1], border_mode=0)(image=image, mask=gt, de=de).values()
        # image, gt,de = albumentations.RandomCrop(*image_size[::-1])(image=image, mask=gt, de=de).values()
        ...
        # de = randomPeper(de) if self.do_randomPeper else Image.fromarray(de)
    ...
    #     assert len(self.images) == len(self.gts) ==len(self.des) and len(self.gts) == len(self.images) ==len(self.des)
    #     images = []
    #     gts = []
    #     des = []
    #     traces = []
    #     for img_path, gt_path, de_path in zip(self.images, self.gts, self.des):
    #         img = Image.open(img_path)
    #         gt = Image.open(gt_path)
    #         de = Image.open(de_path)
    #         if img.size == gt.size == de.size:
...
class test_dataset(data.Dataset):
    def __init__(self, image_root, gt_root, de_root,trace_root,testsize, mean=None, std=None):
        ...
        self.des = [de_root + f for f in os.listdir(de_root) if f.endswith('.tif') or f.endswith('.png')]
        self.traces = [trace_root + f for f in os.listdir(trace_root) if f.endswith('.jpg') or f.endswith('.png')]
        self.images = sorted(self.images)
        self.gts = sorted(self.gts)
        self.des = sorted(self.des)
        self.traces = sorted(self.traces)
        ...
        self.de_transform = transforms.ToTensor()
        self.trace_transform = transforms.ToTensor()
        self.size = len(self.images)
        self.index = 0
    ...
    def load_data(self):
        ...
        trace = self.rgb_loader(self.traces[self.index])
        trace = self.transform(trace).unsqueeze(0)

        gt = self.binary_loader(self.gts[self.index])
        de = self.binary_loader(self.des[self.index])
        ...
        return image, gt,de, trace,name, image_for_post
    ...
    def __getitem__(self, item):
        ...
        image = self.transform(image).unsqueeze(0)
        trace = self.rgb_loader(self.traces[item])
        trace = self.transform(trace).unsqueeze(0)
